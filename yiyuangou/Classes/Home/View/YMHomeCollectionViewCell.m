...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.buyView = [UIButton buttonWithFrame:CGRectMake(frame.size.width - 60 , self.detailLable.tmri_bottom +15, 50,25 ) target:self action:@selector(buyClick) title:@"参与" cornerRadius:2];
        ...
        self.pro = [[YMProgressView alloc] initWithFrame:CGRectMake(10, self.detailLable.tmri_bottom ,frame.size.width - 10 - self.buyView.tmri_width - 15 , 40) ];
        self.pro.centerY = self.buyView.centerY;
        [self.contentView addSubview:self.pro];
        
        self.middleView = [[UIView alloc] initWithFrame:CGRectMake(frame.size.width -1, 0, 0.5, frame.size.height)];
        self.middleView.backgroundColor= [UIColor colorWithHex:@"#EAEAEA"];
        [self.contentView addSubview:self.middleView];
        
        UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, frame.size.height -1, frame.size.width, 0.5)];
...
-(id)configWithMode:(HomeHotList*)list;
{
    ...
    [self.iconView sd_setImageWithURL:[NSURL URLWithString:list.goodsImage] placeholderImage:hotPlaceHolder];
    self.pro.progressView.progress = list.progress?[list.progress intValue]/100.0:0;
    NSString *allstr = [NSString stringWithFormat:@"总量%ld",list.expected];
//    NSString *substr = [NSString stringWithFormat:@"%ld",list.expected];
    
    self.pro.ProgressNumber.text =allstr;
//    [allstr alllString:allstr andallcolor:[UIColor colorWithHex:@"#999999"] andallFont:[UIFont systemFontOfSize:12] subString:substr andColor:[UIColor colorWithHex:@"#DD2727"] andsubFont:[UIFont systemFontOfSize:12]];
    
    NSString *allstr1 = [NSString stringWithFormat:@"剩余%ld",list.expected -list.actually];
    NSString *substr1 = [NSString stringWithFormat:@"%ld",list.expected -list.actually];
    
    self.pro.actuallyLalbe.attributedText = [allstr alllString:allstr1 andallcolor:[UIColor colorWithHex:@"#999999"] andallFont:[UIFont systemFontOfSize:12] subString:substr1 andColor:[UIColor colorWithHex:@"#DD2727"] andsubFont:[UIFont systemFontOfSize:12]];
